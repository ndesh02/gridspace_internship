...
1. I have taken a course on C++, a course on Systems Programming and Concurrency that used C, a course on Data Structures and Algorithms and a course on Digital Computers that taught RISC-V. My course on data structures covered stacks, lists, maps and trees as well as how to design algorithmns and Big O notation. My course on digital computers covered assembly, low level programming and memory management. The systems programming course covered processes and other aspects of low level programming in C.

2. My largest software project was a customer facing mobile app at BTNX that was used for logging covid and drug tests. There were multiple facets to the project: cases where customers would log the results for their own usage, cases where employees would log their results so that they would be automatically sent to their employer, and cases where the results needed to be automatically uploaded into an ATS (Application Tracking System). I also took the initiative to create an external site to interface with some of the customer data (useful for large corportations to track and download drug test results en masse) and created a secure website from scratch in 2 weeks. This involved the use of multiple technologies: Xamarin (used to develop iOS and Android applications), Razor pages (for the external site) and APIs to send requests to the ATS and the development of our own API endpoints.

3. I'd say that I am fairly comfortable with Systems Programming. I am taking a course on it right now, so by the time Winter 2023 comes around I should be fairly proficient. I also know C quite well, and am fairly comfortable with all the lower level aspects such as memory management. I have taken a course on Digital Computers that taught us assembly (RISC-V) so I am understand assembly as well.

4. I have a wide variety of things I like to do in my free time. Recently, I have been reading quite a bit of historical fiction and sci-fi as well as non-fiction. I also enjoy listening to podcasts about modern history. Another hobby of mine is painting. I taught myself how to paint landscapes a year ago and I find it to be a good creative outlet, especially when I am stressed. I have also learnt how to crochet recently and enjoy making a lot of stuffed toys - they make great birthday gifts! One last hobby of mine is playing Stardew Valley - I got into it a few years ago and haven't been able to stop playing since. That sums up what I like to do in my free time.

5. One personal project that I made for fun was a game called Fox vs Bats. I used the open source game engine, Godot, to create this project. The reason I decided to start creating the game was out of a desire to learn how to use a game engine. I had downloaded Unity, but I liked the idea of the open source engine more and all of the fundamental concepts were the same. I learnt about game design and the tradeoffs of frames per second versus computing power consumed. Once I was done, my game had 10 playable levels and I distributed to amongst friends and family. 
...
7. I think that good code isn't littered with comments or overly clever - it should be self explanatory. Comments should be added where explanations are needed, but for the most part the variable names should be clear and the structure apparent. I also think it's best to always take the most straightforward solution (without compromising on efficiency). This will help maximize readability - another feature of good code. It's also important to organize the code in small chunks such as classes, methods and functions that do one thing to optimize reusuability. Finally, good code must actually work. This means that it has been tested and rewritten as needed.
...
8. I've heard great things about Gridspace and would love the opportunity to work for you! I am most excited to develop my machine learning and AI skills since I know that you are creating cutting edge AI technology. I have also heard that this is a fast paced work environment, and I want to develop my problem solving skills. I am just starting out in my career and I know that Gridspace will help me succeed!

9. In addition to what I've learned in school, I've also learned quite a bit over the course of my last three co-ops as well. My first internship at the Engineering Ideas Clinic at the University of Waterloo focused on creating a mini autonomous car. Over the course of this internship, I learned about Machine Learning and how to create CNNs. My second internship took place at BTNX, where I focused on mobile app and web app development. I used Xamarin to create an app for Android and iOS and I created a secure website from scratch to work alongside the app. During my third internship at Descartes, I focused on Full Stack and Backend development using .Net framework and .Net core. I think my previous internship experience will help make me an asset to Gridspace.
...
2. According to the Universal Approximation theorem, neural networks can approximate any continuous function (with at least one hidden layer and some non-linear activation functions). So, this means that they cannot always approximate discontinuous functions. For example, f(x) = 1/(x-5). This function has a discontinuity at 5, so a neural network will not be able to approximate it. This is because the loss function of neural networks uses differentiation. However, it is not possible to differentiate at a discontinuity. So, neural networks can't approximate discontinuous functions. 
...
down.
- Words: In English, many words are spelt the same but have multiple meanings (such as the word letter). In addition to this, many words are spelt differently but are pronounced the same (such as mail and male). My conlang would not have any words like these, as this makes the language much more difficult to learn.

9. With such complex words, the language processing model must be extensively developed. Since so many possible combinations of words arise, it's less feasible to create an entire dictionary of words. This means that the model must be able to "decode" each word and understand the components that go into it. An approach would be to search and analyze documents written in this language and form a collection of texts with accompanying translations for the algorithm to learn from. This way we can teach the processing model how to decode Nahuatl. This languages adds prefixes and suffixes to the words themselves, so it would be good to have one algorithm to remove those and then another to translate the base from and show a translation. Another challenge that arises when processing this language is allowed error. Because the words in this language are so specific, any errors the processing model makes will have a huge impact on the result. As such, the model would have to be extremely accurate.
